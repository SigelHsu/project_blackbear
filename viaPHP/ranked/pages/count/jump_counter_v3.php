...
<script>
	...
	$(document).ready(function() {
		...
		setInterval(() => {
			...
			tmp_OldfullNumberWithMark = ary_sample[x-1]["full"].toLocaleString();
			tmp_NewfullNumberWithMark = ary_sample[x]["full"].toLocaleString();
			ary_sample[x]["indi"] = fun_sepratString(tmp_NewfullNumberWithMark);
			//console.log("ary_sample["+x+"] => ", ary_sample[x]);
			
			console.log(tmp_OldfullNumberWithMark, ": ", tmp_NewfullNumberWithMark);
			let findDiffIndex = fun_findFirstDifferenceIndex(tmp_NewfullNumberWithMark.toString(), tmp_OldfullNumberWithMark.toString());
			...
			for (let key in tmp_indi) {
				if(key >= findDiffIndex) {
					let duration = 0.5 + ( (getLength - key) * 0.1); // 根據位置設置動畫持續時間
